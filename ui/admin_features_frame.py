...
class AdminFeaturesFrame:
    ...
    def show_manage_users_popup(self):
        ...
        ttk.Button(search_frame, text="Tìm kiếm", command=perform_search, bootstyle="primary").pack(side='left', padx=5)
        ...
        def get_selected_username():
            ...
            if not selected_item:
                Messagebox.show_warning("Vui lòng chọn một user để thực hiện thao tác.", "Chưa chọn User", parent=popup)
                return None
        ...
        ttk.Button(button_frame, text="Đặt lại mật khẩu", command=open_reset_password_popup, bootstyle="primary").pack(side='left', padx=5)
        ttk.Button(button_frame, text="Đổi vai trò", command=open_change_role_popup, bootstyle="info").pack(side='left', padx=5)
        ttk.Button(button_frame, text="Đóng", command=popup.destroy, bootstyle="secondary").pack(side='right', padx=5)
    ...
    def show_create_user_popup(self):
        ...
        ttk.Button(button_frame, text="Tạo User", command=submit, bootstyle="success").pack(side="left", padx=10)
    ...
    def _show_reset_password_popup(self, parent, username, callback):
        ...
        def submit():
            ...
            if not new_password:
                Messagebox.show_error("Mật khẩu không được để trống.", "Lỗi", parent=popup)
                return
            database.update_password(username, new_password)
            Messagebox.show_info(f"Đã đặt lại mật khẩu cho {username}.", "Thành công", parent=popup)
            callback()
            popup.destroy()

        button_frame = ttk.Frame(frame)
        button_frame.grid(row=1, columnspan=2, pady=20)
        ttk.Button(button_frame, text="Xác nhận", command=submit, bootstyle="primary").pack(side="left", padx=10)
        ttk.Button(button_frame, text="Hủy", command=popup.destroy, bootstyle="secondary").pack(side="left")
    ...
    def _show_change_role_popup(self, parent, username, callback):
        ...
        def submit():
            ...
            if not new_role:
                Messagebox.show_error("Vui lòng chọn một vai trò.", "Lỗi", parent=popup)
                return
            database.update_user_role(username, new_role)
            Messagebox.show_info(f"Đã cập nhật vai trò cho {username} thành {new_role}.", "Thành công", parent=popup)
            callback()
            popup.destroy()
